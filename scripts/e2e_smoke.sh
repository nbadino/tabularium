...
BASE_URL="${LLOYDS_E2E_URL:-http://127.0.0.1:8787}"
TMP_HTML="$(mktemp /tmp/lloyds-lab-e2e.XXXXXX.html)"
SERVER_PID=""
cleanup() {
  rm -f "$TMP_HTML"
...
if ! curl -fsS "$BASE_URL/api/health" >/dev/null 2>&1; then
  "$ROOT/scripts/run.sh" >/tmp/lloyds-lab-e2e-server.log 2>&1 &
  SERVER_PID=$!
  for _ in {1..30}; do
...
grep -q '<title>Lloyds Lab' "$TMP_HTML"
