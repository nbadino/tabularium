...
_IT: dict[str, str] = {
    ...
    "ssh_host_missing": "manca l'host SSH della GPU remota",
    "ssh_repo_missing": "manca il percorso del repo di training sulla GPU remota (parsing/train)",
    "ssh_python_missing_w": "nessun Python indicato per la GPU remota: lo script attiverà l'ambiente conda monkeyocrv2-train",
    "playground_ocr_unavailable": "nessun motore OCR locale installato (RapidOCR o PaddleOCR)",
    "no_gold_pages": "nessuna pagina approvata da usare come verità: approva nello Studio le pagine corrette",
...
_EN: dict[str, str] = {
    ...
    "ssh_host_missing": "the SSH host of the remote GPU is missing",
    "ssh_repo_missing": "the training repo path on the remote GPU is missing (parsing/train)",
    "ssh_python_missing_w": "no Python given for the remote GPU: the script will activate the monkeyocrv2-train conda env",
    "playground_ocr_unavailable": "no local OCR engine installed (RapidOCR or PaddleOCR)",
    "no_gold_pages": "no approved page to use as ground truth: approve the corrected pages in the Studio",
...
_FR: dict[str, str] = {
    ...
    "ssh_host_missing": "l'hôte SSH du GPU distant manque",
    "ssh_repo_missing": "le chemin du dépôt d'entraînement sur le GPU distant manque (parsing/train)",
    "ssh_python_missing_w": "aucun Python indiqué pour le GPU distant : le script activera l'environnement conda monkeyocrv2-train",
    "playground_ocr_unavailable": "aucun moteur OCR local installé (RapidOCR ou PaddleOCR)",
    "no_gold_pages": "aucune page approuvée à utiliser comme vérité : approuvez les pages corrigées dans le Studio",
